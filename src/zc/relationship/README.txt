...
specify it here by hand.
    ...
    >>> interface.directlyProvides(container, ILocalUtility)
...
cycle.
    ...
    >>> list(o.id for o in container.findTargets(app['ob1'], None))
    ['ob2', 'ob3', 'ob5', 'ob1']
    >>> list(o.id for o in container.findTargets(app['ob2'], None))
    ['ob5', 'ob1', 'ob2', 'ob3']
    >>> list(o.id for o in container.findTargets(app['ob5'], None))
    ['ob1', 'ob2', 'ob3', 'ob5']
...
a ValueError.
    ...
    ...
    ValueError: maxDepth must be None or a positive integer
    >>> container.findTargets(app['ob0'], 'kumquat')
    Traceback (most recent call last):
    ...
    ValueError: maxDepth must be None or a positive integer
...
for some sources.
    ...
    ...
    ValueError: maxDepth must be None or a positive integer
    >>> container.findSources(app['ob0'], 'kumquat')
    Traceback (most recent call last):
    ...
    ValueError: maxDepth must be None or a positive integer
...
Cyclical relationships are returned in a special tuple that implements
...
Cycle paths not only have a marker interface to identify them, but include
a `cycled` attribute that is a frozenset of the one or more objects that
participate in the cycle.  These objects will by definition be in the targets
of the last relationship in the path, and in the sources of a previous
...
It must always take both a source and a target, and defaults to a maxDepth of
1.
...
Setting maxDepth works as usual.
...
to make the cycle.
    ...
    >>> list(container.findTargets(app['ob2'], None)) # before removal
    [<Demo ob5>, <Demo ob1>, <Demo ob2>, <Demo ob3>]
    >>> list(container.findSources(app['ob2'], None)) # before removal
    [<Demo ob1>, <Demo ob0>, <Demo ob5>, <Demo ob2>]
